  ...
  if (table.contains(vals, endStr) or tonumber(endStr)) or (startStr == "mode" and table.contains(modes, endStr)) then
  	...
  	map.setConfigs(matches[2])
		...
		<AliasGroup isActive="yes" isFolder="yes">
			...
			<Alias isActive="yes" isFolder="no">
				<name>Map Move aliases</name>
				<script>move(matches[2])</script>
				<command></command>
				<packageName></packageName>
				<regex>^(s|n|e|w|south|north|east|west|up|down)$</regex>
			</Alias>
		</AliasGroup>
		<AliasGroup isActive="yes" isFolder="yes">
...
function gotoarea(area)
  local area_id = map.findAreaID(area, true)
	local roomTable = getAreaExits(area_id, false)
	if roomTable == nil then
	 echo('Can not find area exits')
	 end
	local path_info = {}
	local best_room = {}
	for i,v in ipairs(roomTable) do
	   echo("room: "..v)
	   local has_path, cost = getPath(getRoomIDbyHash( gmcp.room.info.num),v)
	   --echo ("path: "..has_path)
		 if has_path then
		    if best_room.cost == nil or cost &gt; best_room['cost'] then
				  best_room['path']=speedWalkPath
					best_room['cost']=cost
					best_room['id']=v
				end
		end
	end
	if best_room ~= nil then
		speedWalkPath =best_room['path']
		map.echo('Found path:')
		gotoRoom(best_room.id)
	end
end</script>
			<eventHandlerList />
		</Script>
...
--        map.echo("Prompt Captured",true)
    ...
    elseif event == "mapStop" then
		...
		elseif event =='gmcp.room.wrongdir' then
			raiseEvent("onMoveFail")
		end
end
...
registerAnonymousEventHandler('gmcp.room.wrongdir','map.eventHandler')
...
function map.move(dir)
  if getPlayerRoom() ~= nil then
	  local doors = getDoors(getPlayerRoom())
		if doors ~= nil and doors[dir] ~= nil then
			if doors[dir] == 2 then
				send("unlock "..dir,false)
			end
			send("open "..dir,false)
		end
	end	 
